#!/usr/bin/env python3
"""日本語文字とアルファベット/数字の間に半角スペースを挿入する。

AsciiDoc (.adoc) ファイルを対象に、日本語文字 (ひらがな・カタカナ・漢字) と
ラテン文字/数字 (a-zA-Z0-9) が隣接する箇所に半角スペースを挿入して可読性を
向上させる。

また、半角括弧 ( の前および ) の後に半角スペースを挿入する。
ただし括弧前後が空白・括弧・句読点の場合はスキップする。

全角スペース (U+3000) は半角スペースに変換する。

コードブロック (---- / ....) 内およびバッククォート内のコンテンツは変更しない。
バッククォートと日本語文字の境界にはスペースを挿入する。
"""
...
_jp_lat = re.compile(f"({JP_RANGE})({LAT_RANGE})")
_lat_jp = re.compile(f"({LAT_RANGE})({JP_RANGE})")
_jp_re = re.compile(JP_RANGE)
_before_paren = re.compile(r"([^\s(])\(")
_after_paren = re.compile(r"\)([^\s).,;:!?。、」』】*>])")


def _add_spaces(text: str) -> str:
    text = text.replace("　", " ")
    text = _jp_lat.sub(r"\1 \2", text)
    text = _lat_jp.sub(r"\1 \2", text)
    text = _before_paren.sub(r"\1 (", text)
...
def _process_line(line: str) -> str:
    parts = line.split("`")
    if len(parts) % 2 == 0:
        return _add_spaces(line)

    for i in range(0, len(parts), 2):
        parts[i] = _add_spaces(parts[i])

    result = parts[0]
    for i in range(1, len(parts)):
        if i % 2 == 1:
            if result and _jp_re.match(result[-1]):
                result += " "
            result += "`" + parts[i]
        else:
            result += "`"
            if parts[i] and _jp_re.match(parts[i][0]):
                result += " "
            result += parts[i]
    return result
