...
### 9.20.1 コードブロック内容の事後検証

#### 背景

新規ファイル翻訳 (`_translate_new_file`) はファイル全体を AI に渡すため、コードブロック内のコード本体 (`#` コメント以外の行) が翻訳されてしまうケースがある。例: `CRITICAL: This restore is permanent.` → `重要: このリストアは元に戻せません。`

#### 処理内容

ステップ 10a の事後補正パスに以下を追加する:

1. 各 `.adoc` ファイルをブロックパーサで解析する
2. 対応する `originals/` の EN ファイルも解析する
3. EN と JA の `code_block` ブロックをインデックス順に対応付ける
4. 行数が一致するブロック対のみを処理する (行数不一致は構造変更のためスキップ)
5. 以下の行は比較対象外とする:
   - デリミタ (`----`)
   - 行頭コメント (`#` で始まる行)
   - ブロック属性 (`[` で始まる行)
   - ブロックタイトル (`.` で始まる行)
   - インラインコメント部のみ異なる行 (コード部分は同一で `# comment` 部分のみ翻訳されている)
6. コールアウトマーカー (`# <N>`) が除去されている場合は常に EN の行で復元する
7. その他のコード行は、差分行数がコード行総数の 30% 以下の場合のみ EN の行で復元する (30% 超は upstream 更新による差異と判断しスキップ)
8. 復元が行われた場合、ログに出力する

#### 冪等性

この処理は冪等である。既に EN と一致する行には変更を加えない。
...
`document_header` の翻訳結果を処理する際、EN 原文の `:attr:` 行で翻訳結果に存在しないものを復元する。ただし `:navtitle:` は翻訳対象の属性であるため、JA 側の値を保持し EN の値で上書きしない。その他の属性 (`:experimental:`, `:source-highlighter:` 等) は EN の値で上書きする。
