...
* <<quick-health-check,クイックヘルスチェック>>
* <<method-1-console,メソッド 1: コンソールアクセス>>
* <<method-2-vnc,メソッド 2: VNC アクセス>>
* <<method-3-virtctl-ssh,メソッド 3: virtctl 経由の SSH>>
* <<method-4-nodeport,メソッド 4: NodePort Service 経由の SSH>>
* <<method-5-loadbalancer,メソッド 5: LoadBalancer 経由の SSH>>
* <<method-6-port-forward,メソッド 6: ポートフォワード経由の SSH>>
* <<method-7-secondary-network,メソッド 7: セカンダリーネットワーク経由の直接アクセス>>
* <<diagnostic-commands,調査コマンド>>
...
空の場合、guest agent は実行されていません。注: 基本的なアクセス方法 (コンソール、VNC、SSH、ポートフォワード、NodePort、LoadBalancer、セカンダリーネットワーク) には guest agent は必須ではありません。guest agent はゲスト OS の詳細情報を提供したり、動的な SSH 鍵注入のような機能を有効にしたりするために使用されます。
...
WARNING: この例の cloud-init パスワードはデモ目的のみです。本番環境では常に強力でユニークなパスワードを使用するか、SSH 鍵認証を使用してください。
...
cloudInitNoCloud:
  userData: |
    ...
    chpasswd:
      expire: false
----
. または、SSH 鍵認証を使用し、キーを使用してコンソール経由でアクセスします。
...
- VM spec でグラフィックスデバイスが明示的に除外されている
...
# virtctl 出力でポート番号を確認
virtctl vnc my-vm -n my-namespace
# 表示例: "VNC connection on port 5900"
...
[source,bash,role=execute]
----
# VM に IP アドレスがあることを確認
oc get vmi my-vm -n my-namespace -o jsonpath='{.status.interfaces}'

# VM 内の SSH サービスを確認 (コンソール経由)
virtctl console my-vm -n my-namespace
# VM 内部: systemctl status sshd
----
...
# Endpoints フィールドを確認 — VM Pod の IP が表示されるはず
----

**一般的な原因**:
- Service の Selector が複数の VM に一致している
- Service の Selector がどの VM にも一致していない

**解決策**:

. Service Selector にユニークなラベルを使用します:
+
[source,yaml]
----
selector:
  kubevirt.io/domain: my-vm  # VM ごとに一意
  # または
...
# 任意の Node の INTERNAL-IP または EXTERNAL-IP (利用可能な場合) を使用

# または VM が実行されている特定の Node を取得
...
[source,bash,role=execute]
----
# LoadBalancer プロバイダーが利用可能か確認
oc get svc -A | grep LoadBalancer

# クラスターが LoadBalancer をサポートしているか確認
oc describe svc my-vm-ssh -n my-namespace
----
...
- ローカルポートが既に使用されている
- VMI が実行されていない
- NetworkPolicy がポートフォワードをブロックしている
...
[[method-7-secondary-network]]
== メソッド 7: セカンダリーネットワーク経由の直接アクセス

=== 問題: セカンダリーネットワーク上の VM に到達できない
...
[source,bash,role=execute]
----
# VM にセカンダリーインターフェースが設定されているか確認
oc get vmi my-vm -n my-namespace -o jsonpath='{.status.interfaces}' | jq

# NetworkAttachmentDefinition (NAD) の存在を確認
...
- VM にセカンダリーネットワークインターフェースがない
- セカンダリーネットワークの IP が構成されていない
- 物理ネットワークにアクセスできない
- VLAN 構成のエラー
...
virtctl console my-vm -n my-namespace
# VM 内部:
ip addr show enp2s0  # セカンダリーインターフェース
----
. cloud-init 経由で静的 IP を構成します (cloud-init IP 構成チュートリアルを参照) 。
. VLAN ネットワークについては、OVS bridge 検証ガイドを参照してください。

=== 問題: セカンダリーネットワークインターフェースが VM で表示されない
...
**一般的な原因**:
- NAD が同じ namespace に存在しない
- VM spec にセカンダリーネットワークインターフェースが欠落している
- Multus CNI が動作していない

**解決策**:

. NAD が正しい namespace にあるか確認します:
+
[source,bash,role=execute]
----
oc get network-attachment-definition -n my-namespace
----
. VM spec にネットワークとインターフェースの両方が定義されていることを確認します。
. multus ログを確認します:
+
[source,bash,role=execute]
----
...
これにより、VM がアクセス可能であり、guest agent が動作していることが確認できました。guest agent は追加の OS 情報を提供しますが、SSH ベースのアクセス方法には必須ではありません。
