...
空の場合、guest agentは実行されていません。注: 基本的なアクセス方法 (コンソール、VNC、SSH、ポートフォワード、NodePort、LoadBalancer、セカンダリネットワーク) にはguest agentは必須ではありません。guest agentはゲスト OS の詳細情報を提供したり、動的な SSH キー注入のような機能を有効にしたりするために使用されます。
...
. Service Selectorにユニークなラベルを使用します:
+
[source,yaml]
----
selector:
...
- ローカルポートが既に使用されている
- VMI が実行されていない
- NetworkPolicyがポートフォワードをブロックしている
...
これにより、VM がアクセス可能であり、guest agentが動作していることが確認できました。guest agentは追加の OS 情報を提供しますが、SSH ベースのアクセス方法には必須ではありません。