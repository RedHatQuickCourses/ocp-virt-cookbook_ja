...
注: `your-public-key-here` は実際の SSH 公開鍵に置き換えるか、テスト用にパスワード認証を使用してください。
...
== 手法 6: セカンダリネットワーク経由の直接ネットワークアクセス

bridge ネットワークや localnet ネットワーク上の VM に対して、IP 経由で直接アクセスします。

=== 前提条件

* VM がセカンダリネットワーク (bridge、localnet、VLAN) に接続されていること
