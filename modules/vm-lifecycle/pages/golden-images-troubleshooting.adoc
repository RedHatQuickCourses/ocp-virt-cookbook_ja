...
本ガイドでは、OpenShift Virtualization におけるカスタムOSイメージおよびゴールデンイメージの作成と使用に関連する問題についてのトラブルシューティング手順、確認コマンド、解決策を提供します。
...
- `status.guestOSInfo.id: fedora` (guest agentが実行されていること)
...
期待される出力には `Bound` ステータスと正しいStorage Classが表示されます:
[source,text]
----
...
効率的なクローンが実行されているため、すべてベースVMディスクから派生した複数のPVCを確認してください:
[source,text]
----
NAME                   STATUS   VOLUME           CAPACITY   ACCESS MODES   STORAGECLASS   AGE
...
oc get storageclass
----
. 指定されていない場合、デフォルト Storage Class を確認します:
+
[source,bash,role=execute]
----
